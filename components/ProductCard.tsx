...
export const ProductCard: React.FC<ProductCardProps> = ({ product, isSellerView = false, onEdit, onQuickView, onUpdate, viewMode = 'grid' }) => {
    ...
    if (viewMode === 'shelf') {
        return (
            <div onClick={() => onQuickView?.(product)} className="bg-base-100 dark:bg-gray-800 rounded-md shadow-sm overflow-hidden group flex flex-col relative cursor-pointer h-full transition-shadow duration-300 hover:shadow-lg">
                <div className="relative w-full aspect-square overflow-hidden">
                     <img src={product.images[0]} alt={product.name} className="w-full h-full object-cover" />
                </div>
                <div className="p-1.5 flex flex-col flex-grow">
                    <h3 className="text-xs font-normal text-base-content h-8 leading-tight line-clamp-2">
                        {product.name}
                    </h3>
                    <div className="mt-auto pt-1">
                       <span className="text-sm font-bold text-primary dark:text-blue-400">SLL {formattedPrice}</span>
                    </div>
                </div>
            </div>
        );
